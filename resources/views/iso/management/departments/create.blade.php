    ...
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            <div class="p-6 border-b border-gray-100">
                ...
                <p class="text-sm text-gray-500 mt-1">Register a new office/section in the ISO Document system directory.</p>
            </div>
            ...
            <form action="{{ route('iso.management.departments.store') }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
                @csrf
                    ...
                    <div>
                        ...
                        <select id="parent_code" class="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-red-950 focus:border-red-950 bg-white">
                            ...
                            @foreach($parents as $p)
                                <option value="{{ $p->code }}">{{ $p->code }} - {{ $p->dept }}</option>
                            @endforeach
                        </select>
